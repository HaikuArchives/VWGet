...
logvprintf (enum log_options o, const char *fmt, va_list args)
{
       ...
       /*-------------------------------------------------*/
	   /*ADDED BY SLEMA FOR VISUAL WGET*/

	   vsprintf(tmp,fmt,args); //copy with arguments into a plain string

		if (the_info!=NULL) //we just want to cut the '\n' which display wrong in the titlebar
		{
			if (rindex(tmp,'\n')!=NULL) //if it contains a carriage return, remove it.
			{
			while(tmp[pos+1] != '\n')  {	the_info[pos] = tmp[pos];  pos++; }
			}
		}
	   // instead of just   if (the_info!=NULL) sprintf(the_info,tmp); 
	   /*-------------------------------------------------*/  	   
...
#ifdef HAVE_STDARG_H /* If we have stdarg.h, we'll pretend it's an
			ANSI system.  */
void
