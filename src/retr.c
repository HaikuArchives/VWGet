...
{
 /*REMOVED BY SLEMA FOR VISUAL WGET*/
	  ...
	  if (res >= line_bytes)
	    {
	      ...
	      ndot = 0;
	        ...
	        /*-------------------------------------------------*/
			/*ADDED BY SLEMA FOR VISUAL WGET*/
	 		sprintf(the_info," [ skipping %dK ]",
			 (int) ((nrow * line_bytes) / 1024));
	        current_value =  (int) ((nrow * line_bytes));
	        /*-------------------------------------------------*/
		}
	}
...
//      logprintf (LOG_VERBOSE, "\n%5ldK ->", nrow * line_bytes / 1024);
  ...
  offs += res;
	        ...
	        /*-------------------------------------------------*/
			/*ADDED BY SLEMA FOR VISUAL WGET*/
			if (download_over==0) //si false
			{
			current_value +=res;
			the_time = elapsed_time() / 1000; //coz en milliseconds
			}
 			/*-------------------------------------------------*/
  ...
  for (; offs >= opt.dot_bytes; offs -= opt.dot_bytes)
	...
	/*LOGS REMOVED BY SLEMA FOR VISUAL WGET*/
	      ...
	      const char *refurl, int *dt)
{
  ...
  struct urlinfo *u;
 	   ...
 	   /*-------------------------------------------------*/
	   /*ADDED BY SLEMA FOR VISUAL WGET*/
	   sprintf(the_url,origurl); //get url name
 	   /*-------------------------------------------------*/
