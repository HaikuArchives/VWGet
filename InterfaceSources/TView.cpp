	...
	BView(rect, name, B_FOLLOW_ALL, B_WILL_DRAW | B_PULSE_NEEDED)
{
	...
	speed = new BStringView(BRect(0, 24,
				((Bounds().Width() - 32) / 2) - 48, 24 + 10), "", "(speed)");
	AddChild(speed);

	remaining = new BStringView(
			BRect(((Bounds().Width() - 32) / 2) - 48, 24,
				Bounds().Width() - 32, 24 + 10),
			"", "(remaining time)");
...
{
	if (download_over == false && file_length != 0) {
		...
		if ((the_time - old_time) > 4 ) {
			kbytes_per_second =
				float(((current_value-old_value)) / 1024) / (the_time - old_time);
			old_time = the_time;
...
void TView::DoDisplay()
{
	...
	if (kbytes_per_second > 1024) {
		sprintf(str, "Download speed: %3.2f Mb/sec (WOW!)",
				kbytes_per_second/1024);
	}
	else {
		if (kbytes_per_second > 1)
			sprintf(str,"Download speed: %3.2f kb/sec",
					kbytes_per_second);
		else
			sprintf(str,"Download speed: %3.0f bytes/sec",
					kbytes_per_second * 1024);
	}
	speed->SetText(str);
}
...
void TView::StopDownload()
{
	...
	Window()->PostMessage(new BMessage(UPDATE_TITLE));
	//if the download is over (or arborted) the pause/play button turns into a back button
    SetBackButton();
}
...
{
	//moyenne pendant durée totale download
	kbytes_per_second = (float(current_value) / 1024) / the_time;
	//affichage kb/sec	
	if (kbytes_per_second > 1024) {
		sprintf(str, "Average download speed: %3.2f Mb/sec (WOW!)",
				kbytes_per_second / 1024);
	}
	else {
		if (kbytes_per_second > 1)
			sprintf(str, "Average download speed: %3.2f kb/sec",
					kbytes_per_second);
		else
			sprintf(str, "Average download speed: %3.0f bytes/sec",
					kbytes_per_second * 1024);
	}
	speed->SetText(str);
...
void TView::TimeDisplay()
{
	...
	uint32 rem_seconds =
		uint32((file_length - current_value) / (kbytes_per_second * 1024));
	...
	if (download_over == false)
		sprintf(str,
				"Elapsed time:  %ld h %ld min %ld s     Remaining time: %ld h %ld min %ld s",
				elp_hours,elp_minutes,elp_seconds,rem_hours,rem_minutes,rem_seconds);

	else
		sprintf(str,"Elapsed time:  %ld h %ld min %ld s", elp_hours,elp_minutes,elp_seconds);
...
void TView::AttachedToWindow()
{
	...
	rect.Set(Bounds().Width() - 28, 6,
			Bounds().Width() - 2, Bounds().Height() - 4);
	...
	if (downBitmap == NULL)
		downBitmap = new BBitmap(BRect(0, 0, 26 - 1, 22 - 1), B_RGB32);
	if (upBitmap == NULL)
		upBitmap = new BBitmap(BRect(0, 0, 26 - 1, 22 - 1), B_RGB32);

	// tempview for creating the picture
	BView* tempView = new BView(BRect(0, 0, 26 - 1, 22 - 1),
			"temp", B_FOLLOW_NONE, B_WILL_DRAW);
	AddChild(tempView);
	...
	pause = new BPictureButton(
			rect, "", off, on,
			new BMessage(PAUSE_DOWNLOAD),B_TWO_STATE_BUTTON);
	AddChild(pause);
}
...
void TView::SetBackButton()
{
	...
	pause->RemoveSelf();
	pause =
		new BPictureButton(rect, "", on, on,
				new BMessage(PAUSE_DOWNLOAD)); //oui 2x la même image
	AddChild(pause);
