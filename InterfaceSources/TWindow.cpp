...
TWindow::TWindow(BRect frame)
				: BWindow(frame, "Visual-WGet", B_TITLED_WINDOW, B_NOT_RESIZABLE | B_NOT_ZOOMABLE)
{
...
void TWindow::MessageReceived (BMessage *msg)
{
	...
	switch (msg->what)
	{
		...
		be_app->Unlock();
		init_params();
		//is_command_line =false; //très important après init_params...
		PostMessage(new BMessage(UPDATE_TITLE));
		
		
		//WARNING MESSAGE UNTIL WE GET RID OF THE BUG...
		alert = new BAlert("", "As stated in the docs, there's a bug here... so you'd better quit and restart, downloading won't work anymore...", 
                     "GET LOST!",NULL,NULL, B_WIDTH_FROM_WIDEST, B_INFO_ALERT); 
   		alert->Go();
		...
		if (ptg.paused == false)  
			{ 	
				...
				mainView->progress->Draw(mainView->progress->Bounds()); //sinon la barre se redraw pas
			...
			if (params_entered==true)
				{
					...
					strcat(str," - ");
					strcat(str,the_info);		
					if (error_happened==true) strcat(str," -- DOWNLOAD ABORTED");
					if (server_supports_resume==false) strcat(str,"  >>NO RESUMING!<<");
		//			printf("\nThe title is: %s",str);
					SetTitle(str);
					Unlock();
				}
		...
		is_command_line=false; //très utile pour faire le LCD (pour continue dans le même dossier)
		params_entered=true; //on peut plus fermer sans avertissement après ça 
...
// the bug not allowing to download after having cancelled disappears
		...
		{
			sprintf(strb,"--output-document="); 	strcat(strb,save_name);
			nb_args++;
...
//------------------------------------------------------------------
		...
		case ABOUT_BOX:
  		alert = new BAlert("", 
                     ...
                     "So what?",NULL,NULL, B_WIDTH_FROM_WIDEST, B_INFO_ALERT); 
   		 alert->Go(NULL);
 	  break;
